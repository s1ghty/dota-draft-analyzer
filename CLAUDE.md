...
- `data/` — all pipeline output + hand-curated files, checked into the repo:
  ...
  - `aliases.json` — hero_id → alternate search names (English
    abbreviations/old names, Russian names/nicknames), not in SPEC.md's
    original scope. Generated by `pipeline/tag_aliases.py`.
...
The whole point of the pipeline is that this is the *only* time it touches
the internet — the app itself never does. When Valve ships a patch and the
meta/stats shift, refresh everything with:
...
Run that from the project root. It re-pulls heroes/items/baseline/matchups/
...
- Hand-curated data (hero `tags`/`threat_profile`, item `tier`, aliases)
  **survives the re-run automatically** — the pipeline merges onto the
  existing `data/*.json` rather than overwriting it.
- If a brand-new hero shipped in the patch, the script prints a
  `WARNING: ... untagged` line with that hero's id — they'll work in the
  app immediately (with empty tags/aliases), but won't get tag-driven item
  suggestions or alias search matches until you add an entry for them in
  `pipeline/tag_heroes.py` / `pipeline/tag_aliases.py` and re-run
  `python3 pipeline/tag_heroes.py` / `python3 pipeline/tag_aliases.py`.
- `data/weights.json` is yours to tune whenever the numbers feel off for
  your role — the pipeline never touches it.
- If OpenDota is having a bad day (times out on a handful of heroes), the
  pipeline skips them, tells you which ids failed, and still writes
  everything it *did* get. Just re-run the same command again -- it
  refetches everything from scratch (another 10-20 min), which is slower
  than strictly necessary but always safe. This actually happened once
  during development (OpenDota had a real outage) and took three re-runs to
  fully fill in `item_builds.json`.

No coding, no asking me — just the one command.
...
- Ring Master, Largo, Kez, and Muerta in `tag_heroes.py` were originally
  low-confidence guesses (too new to know well) but got re-tagged from
  actual sourced ability text later — see the inline comments on those
  entries for what they're based on. Still worth a sanity read since that
  was a web search summary, not first-hand tooltip text.
- `pipeline/tag_aliases.py` (aliases.json) is sourced from Valve's own
  official alternate hero-search names for the Russian client (reported by
  cybersport.ru) plus a community glossary for gaps -- solid for English
  abbreviations/old hero names, but flag anything Russian that looks off,
  regional slang drifts and I can't verify it against the live game.
